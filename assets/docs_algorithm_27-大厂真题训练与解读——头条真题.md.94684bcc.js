...
</code></pre><h3 id="思路拓展-1" tabindex="-1">思路拓展 <a class="header-anchor" href="#思路拓展-1" aria-label="Permalink to &quot;思路拓展&quot;">​</a></h3><p>这道题还可以用递归来做。 想一想，怎么实现？</p><h2 id="柱状图中的最大矩形" tabindex="-1">柱状图中的最大矩形 <a class="header-anchor" href="#柱状图中的最大矩形" aria-label="Permalink to &quot;柱状图中的最大矩形&quot;">​</a></h2><blockquote><p>题目描述：给定 n 个非负整数，用来表示柱状图中各个柱子的高度。每个柱子彼此相邻，且宽度为 1 。<br> 求在该柱状图中，能够勾勒出来的矩形的最大面积。</p></blockquote><p><img src="`+o+'" alt=""> 以上是柱状图的示例，其中每个柱子的宽度为 1，给定的高度为 [2,1,5,6,2,3]。<br><img src="'+p+'" alt=""> 图中阴影部分为所能勾勒出的最大矩形面积，其面积为 10 个单位。</p><blockquote><p>示例:<br> 输入: [2,1,5,6,2,3]<br> 输出: 10</p></blockquote><p><strong>命题关键字：数学问题、模拟、单调栈</strong></p><h3 id="思路分析-2" tabindex="-1">思路分析 <a class="header-anchor" href="#思路分析-2" aria-label="Permalink to &quot;思路分析&quot;">​</a></h3><p>首先，矩形面积如何计算？长X宽，对吧？这道题给到我们的是一个高度数组，对于每个高度来说，以它为高度的矩形的宽度是未知的。因此我们最直观的一个思路，就是固定一个高度，去探索宽度的上限。举个例子，假如说我固定的是图中的第一个柱子的高度：</p><p><img src="'+i+'" alt=""><br> 从第一个柱子出发，往前遍历，发现下一个柱子的高度是1。1&lt;2，很明显以第一个柱子为高的矩形宽度没办法再扩散了，它的宽度只能是1。</p><p>再看第二个柱子：</p><p><img src="'+h+'" alt=""></p><p>这个柱子高度是1，我们往下遍历，发现它后面的所有柱子 都比它高，这就意味着以1为高度的矩形面积是可以向下扩散的：</p><p><img src="'+c+'" alt=""><br> 扩散到不能再扩散为止时，已经跨越了5个柱子。现在再回头看，发现它左边的柱子也比自己高，那么矩形的宽度还可以再向左扩散：</p><p><img src="'+g+`" alt=""> 如此一来，以第二个柱子为高的矩形，最大面积可以达到1X6=6。</p><p>由此我们也可以总结出矩形宽度最大值的计算规则：若下一个柱子比当前柱子高，则持续扩散以当前柱子为高度的矩形宽度（扩展矩形的右边界）；否则停止扩散，“回头看”寻找左边界，进而计算总宽度。</p><p>秉持上述的计算规则，对每一个柱子都重复此操作，我们就能得到每一个柱子所支撑的最大矩形的面积。从这些面积中对比出一个最大值，就算是把这道题做出来了。</p><p>基于这个思路，我们来写代码：</p><h3 id="编码实现-2" tabindex="-1">编码实现 <a class="header-anchor" href="#编码实现-2" aria-label="Permalink to &quot;编码实现&quot;">​</a></h3><pre><code>/**
...
</code></pre><h3 id="思路拓展-2" tabindex="-1">思路拓展 <a class="header-anchor" href="#思路拓展-2" aria-label="Permalink to &quot;思路拓展&quot;">​</a></h3><p>这道题还可以用栈来做，但是解法相对比较难推导，这里我先给大家做一遍。</p><p><img src="`+d+`" alt=""><br> 站在高度为6的柱子这里，向后遍历，发现2比6小，矩形宽度不能再扩散了。此时我们回头看，首先计算出来的是高度为6的矩形的面积，然后才计算出来高度为5的矩形的面积——后面的柱子比前面的柱子先出结果。这叫啥？这叫后进先出！后进先出的数据结构是啥？是栈！由此，这道题的大方向就有了个脉络——借助栈来模拟矩形宽度的探索过程。</p><p>具体需要一个什么样的栈呢？回到上一个解法中去看，当柱子高度递增时，我们不做特殊处理（此时只需要入栈）。只有当发现柱子的高度回落时，才会开始“弹出”前面柱子对应的结果（出栈）。所以我们在编码层面的一个基本思路，就是去维护一个单调递增栈。<br> 多说无益，都在注释里了：</p><h3 id="编码实现-3" tabindex="-1">编码实现 <a class="header-anchor" href="#编码实现-3" aria-label="Permalink to &quot;编码实现&quot;">​</a></h3><pre><code>/**
 * @param {number[]} heights
 * @return {number}
 */
const largestRectangleArea = function(heights) {
...
</code></pre>`,89),m=[b];function A(x,k,f,w,P,q){return n(),r("div",null,m)}const N=t(u,[["render",A]]);export{v as __pageData,N as default};
